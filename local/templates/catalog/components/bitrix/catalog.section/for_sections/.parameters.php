...
$obSections = CIBlockSection::GetList(['left_margin' => 'asc'], $filterSections, true, [], false);
while ($arSections = $obSections->GetNext()) {
    if ($arSections['RIGHT_MARGIN'] - $arSections['LEFT_MARGIN'] < 2) {
        $sectionList[$arSections['ID']] = $arSections['ID'].' - '. $arSections['NAME'];
    }
};

//$arTemplateParameters['current_value'] = array(
//    'PARENT' => 'BASE',
